...
function createBridgeServers() {
  ...
  const server = http.createServer((req, res) => {
    ...
    if (req.method === 'GET' && urlPath === '/generation-tasks') {
      const tasks = [...gifGeneration.getGenerationTasks().entries()].map(([taskId, t]) => ({
        taskId,
        status: t.status,
    ...
    if (req.method === 'GET' && urlPath.startsWith('/generation-tasks/')) {
      ...
      const t = gifGeneration.getGenerationTasks().get(taskId);
      if (!t) {
        res.writeHead(404, { 'Content-Type': 'application/json' });
    ...
    if (req.method === 'POST' && urlPath === '/action-sets/generate-reference') {
      ...
      req.on('end', () => {
        try {
          ...
          gifGeneration.getGenerationTasks().set(taskId, {
            status: 'pending',
            progress: 0,
            startedAt: Date.now(),
            kind: 'reference',
